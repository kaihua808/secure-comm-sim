...
QWidget     { background: #1e1e2e; color: #cdd6f4; font-family: "Menlo", "Consolas", monospace; }
...
QPushButton#btn_alice { background: #45475a; color: #a6e3a1; border-color: #a6e3a1; }
QPushButton#btn_alice:hover { background: #a6e3a1; color: #1e1e2e; }

QPushButton#btn_bob { background: #45475a; color: #89b4fa; border-color: #89b4fa; }
QPushButton#btn_bob:hover { background: #89b4fa; color: #1e1e2e; }

QPushButton#btn_danger { background: #45475a; color: #f38ba8; border-color: #f38ba8; }
QPushButton#btn_danger:hover { background: #f38ba8; color: #1e1e2e; }

QPushButton#btn_reset { background: #45475a; color: #fab387; border-color: #fab387; }
QPushButton#btn_reset:hover { background: #fab387; color: #1e1e2e; }
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("安全通信模拟系统  ·  ECC + AES-GCM + ECDSA")
    ...
    def _build_ui(self):
        ...
        lbl_sub   = QLabel("ECC P-256  ·  ECDSA  ·  ECDH  ·  AES-256-GCM  ·  SHA-256")
    ...
    def _gen_keys(self):
        ...
        self._log("✓ Alice 和 Bob 的 ECC P-256 密钥对已生成。", "#a6e3a1")
        self._log(f"  Alice 公钥（前64字节）: {alice_pub_hex[:64]}…", "#6c7086")
        self._log(f"  Bob   公钥（前64字节）: {bob_pub_hex[:64]}…",   "#6c7086")
    ...
    def _alice_encrypt_and_sign(self):
        ...
        self._log("── Alice 发送流程 ──────────────────────", "#89dceb")

        # Step A：AES-GCM 加密消息
        nonce, ciphertext = aes_gcm_encrypt(session_key, message)
        self._log(f"① AES-256-GCM 加密消息", "#a6e3a1")
        self._log(f"   Nonce     : {nonce.hex()}", "#6c7086")
        self._log(f"   密文(hex) : {ciphertext.hex()[:64]}…", "#6c7086")

        # Step B：ECDH 保护会话密钥
        ephemeral_pub_bytes, wrap_nonce, wrapped_key = \
            ecdh_encrypt_session_key(self.bob_pub, session_key)
        self._log(f"② ECDH + AES-GCM 加密会话密钥（保护密钥）", "#a6e3a1")
        self._log(f"   临时公钥  : {ephemeral_pub_bytes.hex()[:64]}…", "#6c7086")
        self._log(f"   包装密文  : {wrapped_key.hex()[:64]}…", "#6c7086")

        # Step C：SHA-256 摘要
        digest = sha256_digest(message)
        self._log(f"③ SHA-256 摘要 : {digest.hex()}", "#a6e3a1")

        # Step D：ECDSA 签名
        signature = ecdsa_sign(self.alice_priv, message)
        self._log(f"④ ECDSA 签名   : {signature.hex()[:64]}…", "#a6e3a1")
        ...
        self._log("✓ Alice 完成加密和签名，数据包已准备就绪。", "#a6e3a1")
    ...
    def _bob_decrypt_and_verify(self):
        ...
        if p.ciphertext is None:
            self._log("数据包为空，请先完成 Alice 发送流程。", "#f38ba8")
            return

        self._log("── Bob 接收流程 ──────────────────────", "#89dceb")
        result_lines = []
        success = True
        ...
        try:
            ...
            self._log(f"① 解密会话密钥成功 : {recovered_key.hex()}", "#89b4fa")
            result_lines.append(f"解密会话密钥 : {recovered_key.hex()}")
        except Exception as e:
            self._log(f"① 解密会话密钥失败 : {e}", "#f38ba8")
        ...
        try:
            ...
            self._log(f"② AES-GCM 解密消息成功 : {dec_text}", "#89b4fa")
            result_lines.append(f"\n解密消息     : {dec_text}")
        except Exception as e:
            self._log(f"② AES-GCM 解密消息失败 : {e}", "#f38ba8")
            result_lines.append(f"\n解密消息     : 失败 ❌  （消息可能被篡改）")
            success = False
            self._show_bob_result(result_lines, False)
            return
        ...
        if sig_ok:
            self._log("③ ECDSA 签名验证通过 ✓", "#a6e3a1")
            result_lines.append(f"\nECDSA 验签   : 通过 ✓")
        else:
            self._log("③ ECDSA 签名验证失败 ✗", "#f38ba8")
            result_lines.append(f"\nECDSA 验签   : 失败 ✗")
            success = False
    ...
    def _tamper(self):
        if self.packet.ciphertext is None:
            self._log("数据包为空，无法篡改。", "#f38ba8")
            return
        ct = bytearray(self.packet.ciphertext)
        ct[0] ^= 0xFF  # 翻转第一个字节
        self.packet.ciphertext = bytes(ct)
        self._log("⚠ 已篡改密文第一个字节（XOR 0xFF），请点击 Bob 解密验证。", "#f38ba8")
        self.txt_packet.setPlainText(
    ...
    def _test_wrong_privkey(self):
        ...
        try:
            ...
            self._log("错误私钥解密：异常成功（不应发生）", "#f38ba8")
            result = "结果：解密成功（异常，不应发生）"
        except Exception:
            self._log("错误私钥解密：解密失败 ✗  ← 符合预期，无法恢复会话密钥。", "#fab387")
